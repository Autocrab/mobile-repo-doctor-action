...
function extractFontFamily(relativePath) {
  const basename6 = relativePath.substring(relativePath.lastIndexOf("/") + 1);
  const nameWithoutExt = basename6.substring(0, basename6.lastIndexOf("."));
...
async function checkSensitiveFiles(context) {
  ...
  const matchedByName = allFiles.filter((f) => {
    const basename6 = import_path6.default.basename(f.relativePath);
    return TIER1_NAMES.includes(basename6) || TIER2_NAMES.includes(basename6);
  });
  for (const file of matchedByName) {
    const basename6 = import_path6.default.basename(file.relativePath);
    const isTier1 = TIER1_NAMES.includes(basename6);
    findings.push({
      ...
      description: isTier1 ? `The file "${file.relativePath}" appears to contain sensitive data (matched name: "${basename6}").` : `The file "${file.relativePath}" is a common mobile config file (matched name: "${basename6}"). Usually not a secret, but verify it doesn't contain sensitive overrides.`,
      ...
      evidence: [{ path: file.relativePath, detail: `Sensitive file name: ${basename6}` }],
      suggestedFix: isTier1 ? `Remove "${basename6}" from version control, rotate any exposed credentials, and add the file to .gitignore.` : `Verify "${basename6}" does not contain production secrets. Consider adding to .gitignore if it holds environment-specific values.`,
...
async function checkModuleInventory(context) {
  ...
  for (const file of allFiles) {
    const basename6 = import_path7.default.basename(file.relativePath);
    const markerType = MODULE_MARKERS[basename6];
    if (markerType) {
      if (!grouped[markerType]) grouped[markerType] = [];
      grouped[markerType].push(file.relativePath);
      continue;
    }
    if (file.relativePath.endsWith(XCODEPROJ_EXTENSION) || basename6.endsWith(XCODEPROJ_EXTENSION)) {
...
var API_BASE_URL = process.env.REPO_DOCTOR_API_URL || "https://repo-doctorbackend-production.up.railway.app";
...
async function reportUsage(apiKey, payload) {
  try {
    ...
    if (res.status === 403) {
      ...
      if (body.error === "repo_limit_exceeded") {
        return { recorded: false, repoLimitExceeded: true, message: body.message };
      }
...
// packages/cli/src/lib/usage.ts
async function sendUsageReport(apiKey, repoPath, stack, score, findingCounts, durationMs) {
  const payload = {
    toolVersion: TOOL_VERSION,
    repoHash: hashRepoPath(repoPath),
    repoFingerprint: hashRepoPath((0, import_node_path2.basename)(repoPath)),
  ...
  return reportUsage(apiKey, payload).catch(() => ({ recorded: false }));
}
...
async function scanCommand(pathArg, options) {
  const repoPath = (0, import_node_path3.resolve)(pathArg);
  try {
  ...
  const usagePromise = apiKey && options.telemetry !== false && !options.offline ? sendUsageReport(apiKey, repoPath, detection.stack, {
    overall: score.overall,
    size: score.axes.size,
    speed: score.axes.speed,
    stability: score.axes.stability,
    hygiene: score.axes.hygiene
  }, {
  ...
  }, durationMs) : Promise.resolve({ recorded: false });
  ...
  if (scanOptions.output) {
    ...
    const reportData = {
      repoName: (0, import_node_path3.basename)(repoPath),
      repoPath,
      stack: detection.stack,
    ...
    for (const fmt of formats) {
      const basePath = (0, import_node_path3.resolve)(scanOptions.output).replace(/\.(html|json)$/, "");
      const ext = fmt === "json" ? ".json" : ".html";
      const outputPath = basePath + ext;
      (0, import_node_fs2.mkdirSync)((0, import_node_path3.resolve)(outputPath, ".."), { recursive: true });
      if (fmt === "json") {
  ...
  if (usageResult && "repoLimitExceeded" in usageResult && usageResult.repoLimitExceeded) {
    console.error(source_default.yellow(`\u26A0 Repo limit exceeded: ${usageResult.message ?? "Usage was not recorded. Upgrade your plan for more repos."}`));
  }
...
program2.name("repo-doctor").description("Mobile Repo Doctor \u2014 analyze mobile repository health").version("0.2.0");
